...
const OthersReport = () => {
  ...
  const handleDownload = () => {
    ...
    const formattedData = filteredData.map((item, index) => {
      ...
      // Format others_items for display
      const othersItemsText = Array.isArray(item.others_items)
        ? item.others_items
            .map(
              (otherItem) => `${otherItem.description}: Rs.${otherItem.amount}`
            )
            .join("; ")
        : "N/A";
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Others Reports");
    ...
    link.download = "others_reports.xlsx"; // Set the filename for the download
  ...
  const handlePrint = () => {
    ...
    const tableHTML = `
      <html>
        <head>
          <title>MDC Others Billing Report</title>
          <style>
            table {
              width: 100%;
          ...
          <table>
            ...
            <tbody>
              ${filteredData
                .map(
                  ...
                  <tr>
                    ...
                    <td>${
                      item.age?.year || item.age?.months || item.age?.days
                        ? `${item.age.year || 0} years, ${
                            item.age.months || 0
                          } months, ${item.age.days || 0} days`
                        : "N/A"
                    }</td>
                    <td>${item.sex}</td>
                    ...
                    <td>
                      ${
                        Array.isArray(item.others_items)
                          ? item.others_items
                              .map(
                                (otherItem) =>
                                  `${otherItem.description}: Rs.${otherItem.amount}`
                              )
                              .join("<br>")
                          : "N/A"
                      }
              ...
              <tr>
                ...
                <td style="text-align: right;"><strong>${grandTotal.total_amount.toFixed(
                  2
                )}</strong></td>
                <td style="text-align: right;"><strong>${grandTotal.amount_paid.toFixed(
                  2
                )}</strong></td>
                <td></td>
              </tr>
  ...
  const calculateGrandTotal = () => {
    return filteredData.reduce(
      (totals, item) => {
        return {
          total_amount:
            totals.total_amount + parseFloat(item.total_amount || 0),
  ...
  ${
    Array.isArray(item.others_items) && item.others_items.length > 0
      ? item.others_items
          .map(
            (otherItem) => `
              <tr>
                <td style="text-align: center;">${
                  otherItem.description || "N/A"
                }</td>
            ...
            <TableBody>
              {filteredData.map((item, index) => (
                <TableRow key={item.billing_no}>
                  ...
                  <TableCell>{item.father_phone_number}</TableCell>
                  <TableCell>{item.mother_phone_number}</TableCell>
                  <TableCell>
                    {Array.isArray(item.others_items) &&
                    item.others_items.length > 0
                      ? item.others_items.map((otherItem, i) => (
                          <div key={i}>
                            {otherItem.description}: ₹{otherItem.amount}
                          </div>
                        ))
                      : "N/A"}
                  </TableCell>
            ...
            <tfoot>
              <TableRow>
                ...
                <TableCell style={{ textAlign: "right", fontWeight: "bold" }}>
                  {calculateGrandTotal().total_amount.toFixed(2)}
                </TableCell>
                <TableCell style={{ textAlign: "right", fontWeight: "bold" }}>
                  {calculateGrandTotal().amount_paid.toFixed(2)}
                </TableCell>
